...
const AdminVetsScreen = () => {
  ...
  return (
    <AdminLayout
      ...
      headerRight={
        <Button onClick={openInvite} style={{ background: "#7B5EA7" }} className="text-white hover:opacity-90">
          <PlusIcon className="w-4 h-4 mr-1" /> Invite New Vet
        </Button>
      }
    >
      <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "#F5F1EC" }}>
                ...
                <tr key={v.id} className="border-t" style={{ borderColor: "#F0EDF5" }}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2 flex-wrap">
                      ...
                      <button onClick={() => openEdit(v)} className="text-[11px] px-2.5 py-1 rounded-full border bg-white inline-flex items-center gap-1" style={{ borderColor: "#F5F1EC" }}>
                ...
                <div className="flex flex-wrap gap-2">
                  {SPEC_OPTIONS.map((s) => (
                    <button
                      ...
                      style={form.specialisations.includes(s) ? {} : { borderColor: "#F5F1EC" }}
                    >
                      {s}
                    </button>
                  ))}
